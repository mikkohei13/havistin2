...
#                new_photo['rawdata'] = photo
                ...
                photo_count = photo_count + 1
                if photo_count >= photo_count_toget:
#                    common_helpers.print_log("Breaking at iNat photos, nro " + str(photo_count)) # debug
                    break
...
# Without caching (caching done by Redis on page-level)
def get_photos_data(qname, max_age_seconds, max_photo_count):
    photos_data = generate_photos_data(qname, max_photo_count)
    return photos_data
...
def main(qname_untrusted):
    ...
    qname = common_taxa.valid_qname(qname_untrusted)
